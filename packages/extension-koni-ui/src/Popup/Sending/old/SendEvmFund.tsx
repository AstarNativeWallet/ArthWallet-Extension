...
function SendEvmFund ({ api, apiUrl, className = '', currentAccount, isEthereum, networkKey, setWrapperClass }: ContentProps): React.ReactElement {
  const { t } = useTranslation();
  const propSenderId = currentAccount?.address;
  const [amount, setAmount] = useState<BigInt | undefined>(BN_ZERO);
